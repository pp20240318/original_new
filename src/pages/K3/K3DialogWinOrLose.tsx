...
export function K3DialogWinOrLose({
  open,
  drawResult,
  // userProfit,
  currencySymbol,
  ...
  open: boolean;
  drawResult?: DGetCurrentThreeKingResult | null;
  ...
  const isWin = useMemo(() => {
    return typeof drawResult?.winAmount === "number" && drawResult?.winAmount > 0;
  }, [drawResult?.winAmount]);

  const result = useMemo(() => {
    if (!drawResult) return;
    const num = drawResult.result;
    const bigOrSmall = drawResult.bigOrSmall.toLowerCase();
    const oddOrEven = drawResult.oddOrEven.toLowerCase();
    
    return {bigOrSmall, oddOrEven, num };
  }, [drawResult]);
            ...
            <div className="absolute top-0 left-0 right-0 bottom-0 flex flex-col items-center pt-40">
              ...
              <div className="flex items-center gap-2 text-white text-sm font-medium mt-9">
                ...
                {result && (
                  <>
                    ...
                    {result && result.num.split(",").map((num, index) => (
                      <span key={index} className="flex justify-center items-center w-7 h-7 rounded-full bg-[#40ad72]">
                        {num} {/* 输出每个拆分后的数字 */}
                      </span>
                    ))}
                    ...
                    <span
                      className={clsx(
                        "flex justify-center items-center h-7 px-2 rounded-md bg-[#40ad72]",
                        result.bigOrSmall === "big" ? "bg-[#ffc511]" : "bg-[#6da7f4]"
                      )}
                    >
                      {t(`wingo1_${result.bigOrSmall}`)}
                    </span>
                    <span
                      className={clsx(
                        "flex justify-center items-center h-7 px-2 rounded-md bg-[#40ad72]",
                        result.oddOrEven === "odd" ? "bg-[#40ad72]" : "bg-[#fd565c]"
                      )}
                    >
                      {t(`wingo1_${result.oddOrEven}`)}
                    </span>
                  </>
                )}
              </div>
              <div className="flex flex-col items-center mt-10">
                ...
                <span className="text-[#6b6b6b] text-sm font-medium mt-2">
                  {t`wingo1_period`}: {drawResult?.opendate}
                </span>
              </div>
